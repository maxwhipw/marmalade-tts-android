...
class MarmaladeTtsServiceTest {
    ...
    fun setUp() {
        ...
        fakeDao = FakeVoiceMetaDao(KittenVoiceCatalog.voices)
        service = MarmaladeTtsService()
        // Inject by reflection — Hilt isn't running so the @Inject lateinit
        // vars are unset. Both fields are public-by-Kotlin-default backing
        // properties; reflection bypasses the lateinit "isInitialized" guard.
    ...
    fun onSynthesizeText_happyPath_callOrderIsStartAudioAvailableDone() {
        ...
        fakeEngine.nextPcm = ShortArray(sampleCount) { (it and 0xFF).toShort() }
    ...
    fun onSynthesizeText_engineNotInstalled_callsErrorExactlyOnce() {
        fakeEngine.synthesizeException = EngineNotInstalledException("kitten")
    ...
    private fun newRequest(text: String): SynthesisRequest {
        ...
        // service falls back to KittenVoiceCatalog.DEFAULT_VOICE_ID — that's
        // fine; the catalog is seeded into the fake DAO in @Before.
