...
object EngineCatalog {
     ...
     *
     * Sizes are taken from the Sherpa-ONNX HF mirror's file listing
     * documented in [LICENSES/kitten-tts.md]. Hashes are PENDING (see
     * SHA256_PENDING) — populated by the first installer run on a verified
     * device or via the generator script.
     */
    private val KITTEN_TOP_LEVEL: List<EngineFile> = listOf(
        ...
        EngineFile(
            ...
            sha256 = SHA256_PENDING,
            sizeBytes = 16_000L, // ~16 KB (8 speakers × ~2 KB embedding)
        ),
        EngineFile(
            relativePath = "tokens.txt",
            url = "$KITTEN_HF_BASE/tokens.txt",
            sha256 = SHA256_PENDING,
            sizeBytes = 1_500L, // ~1.5 KB phoneme vocab
        ),
    )

    /**
     * espeak-ng-data manifest. Generated externally — see
     * [scripts/generate-kitten-manifest.py]. v0.1 lands with the
     * top-level subset hard-coded below; the full ~355-file enumeration is
     * tracked as a STUBS.md item.
     *
